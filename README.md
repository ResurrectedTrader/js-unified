...
| Tests | one suite, written once against `ub::`: 398 cases, green on both backends, every case compared backend against backend with no divergences |
...
The number `ctest` prints is a little larger than 398 and depends on the tree,
...
built). **398 cases is the figure that means the same thing everywhere** - it is
what the test binary itself reports, on either backend. The assertion count is not: a case may assert a
different number of times on each engine, so V8 counts 10789 and SpiderMonkey
10300, and neither number is the one to compare a run against.
